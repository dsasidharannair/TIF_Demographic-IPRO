...
TIF Transfers:
Our original plan was not only to analyze the TIF transfer data from the [Yates TIF dataset](https://github.com/philipayates/chicago2022TIF), but to also analyze the TIF transfers that the city planned ahead of time. These could be found in various "TIF District Programming" files hosted by the Chicago Data Portal in PDF format and hosted by [Data.Gov in CSV format](https://catalog.data.gov/dataset/tif-district-programming-2022-2026). The amount of time and effort required to parse through the thousands of planned transfers from 2014-2026 in a programmatic fashion turned out to be too time consuming. As Innovation Day approached, we had to scrap this portion of the project and focus our attention on parsing the TIF transfer Data from Phillip Yates. 

After our Data Processing stage, we created several heatmaps to visualize the TIF transfer data from 2014-2022 which can be found above with explanations. We wanted to use this data to create models to see if there was a correlation between the demographic makeup of a TIF district, and the amount of money transferred in or out from the district. We trained a Multi-Linear Regression model which had a very high loss value and as seen from the plotted regression lines above. We also trained a Multinomial Logistic Regression model to classify if a district would recieve a positive or negative/no transfer. The classifier ended up predicting all test cases as negative/no transfer which was also useless to us. In conclusion, there was no correlation to be found between the TIF transfer data and the TIF Demographic Data that we collected. We wanted to publish the TIF Transfer maps so that anyone analyzing our results in the future can pick up from where we left off and uncover a correlation that we missed during our time this Semester.
...
- [TIF Transfer Files](https://github.com/dsasidharannair/TIF_Demographic-IPRO/tree/main/Transfers)
	...
	- [TIF Transfer Model Training](https://github.com/dsasidharannair/TIF_Demographic-IPRO/blob/main/Transfers/Correlation_search.ipynb)
